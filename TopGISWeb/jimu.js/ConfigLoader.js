      ...
      ********************************************************/
      ...
      _tryLoadConfig: function () {
        ...
        if (this.urlParams.config) {
          ...
          sigle_USER_ID = window.usercode;
          var getfunUrl = "/Nokia_TopGis_Api/api/authority/getGisConfig";
          if (sigle_USER_ID != "") {
            //var userId=this.urlParams.userId;
            return xhr(getfunUrl + "?userCode=" + sigle_USER_ID, {
              handleAs: 'json',
              method: 'GET',
              headers: headStr
            }).then(lang.hitch(this, function (appConfig) {
              var jsonBaseMap = appConfig.map.basemaps;
              for (var i = 0; i < jsonBaseMap.length; i++) {
                var curip = window.location.host;
                var revUrl = jsonBaseMap[i].url;
