...
function(declare, lang, array, html, Deferred, topic, Evented, on, aspect,
  ...
  clazz = declare(Evented, {
    ...
    constructor: function() {
      ...
      if (window.isBuilder) {
        topic.subscribe("app/layer_checked", lang.hitch(this, this._onLayerChecked));
      } else {
        topic.subscribe("layer_checked", lang.hitch(this, this._onLayerChecked));
      }
    ...
    //下面是新增加的函数
    ,
    _onLayerChecked: function (_layerConfig, _layerVisible) {
        var layerConfig = lang.clone(_layerConfig);
        this.lastLayerConfig = layerConfig;
        array.forEach(this.loaded, function (w) {
            if (!w) {
                //widget maybe deleted in the handler of appConfigChange event
                return;
            }
            w.onLayerChecked(layerConfig, _layerVisible); //update 2017/11/17
        }, this);
    }
  });
