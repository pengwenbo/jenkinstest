...
define([
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
        ...
        _uploadFileBase: function (loadfile, date, uid) {
            ...
            //console.log(data)
            xhr(this._dataRequestUrl+'/ExportExcelData', {
                method: 'POST',
                data: data,
                handleAs: 'json'
            }).then(lang.hitch(this, function (uploadstatu) {
                //console.log(uploadstatu.status);
                if (uploadstatu.status == '1') {
                    popdata = this._getattr(date, uuid);
                } else if (uploadstatu.status == '-1') {
                    alert("上传失败");
                };
            }));
        },
        ...
        _getattr: function (date, uid) {
            //取右侧侧边栏数据
            let url = this._dataRequestUrl+'/GeDataList?'
            url += 'date_id=' + date + '&uuid=' + uid
            xhr(url, {
                method: 'GET'
